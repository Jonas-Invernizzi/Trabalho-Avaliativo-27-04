...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT c.*, p.nome as especie, p.numero_dex, p.tipo_principal, p.tipo_secundario, p.imagem_url 
        FROM capturas c 
    ...
    // Busca a lista de desejos (espécies que ele quer ter)
    $stmt = $pdo->prepare("
    ...
    // Busca as ofertas de troca que ele criou
    $stmt = $pdo->prepare("
        SELECT ot.id,
